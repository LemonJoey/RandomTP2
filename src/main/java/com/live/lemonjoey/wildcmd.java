...
public class wildcmd implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if(sender instanceof Player) {
            ...
            if (args.length == 1) {
                ...
                else if (args[0].equalsIgnoreCase("group")) group((Player) sender);
                else if (args[0].equalsIgnoreCase("cancel")) groupCancel((Player) sender);
        ...
        if(player.hasPermission("randomtp.use")) {
            ...
            if (yml.getInt(player.getUniqueId().toString()) < times || times == -1) {
                ...
                if (plugin.getConfig().getStringList("allowedworlds").contains(player.getWorld().getName())) {
                    ...
                    player.sendMessage(Objects.requireNonNull(message.getString("teleport")).replace("%n", "\n"));
        ...
        if(player.hasPermission("randomtp.reset")) {
            ...
            if(args.length > 1) {
                ...
                if(Bukkit.getServer().getPlayer(args[1]) != null) {
                    ...
                    yml.set(Objects.requireNonNull(Bukkit.getServer().getPlayer(args[1])).getUniqueId().toString(), 0);
                    try {
                        yml.save(ymlFile);
                    } catch (IOException e) {
                        e.printStackTrace();
    ...
    private void groupCancel(@NotNull Player player) {
    ...
    private void group(@NotNull Player player) {
                ...
                if(yml.getInt(player.getUniqueId().toString()) < times || times == -1) {
                    ...
                    plugin.getServer().getScheduler().scheduleSyncDelayedTask(
                            plugin, () ->{
                                ...
                                YamlConfiguration taskyml = YamlConfiguration.loadConfiguration(taskymlFile);
                                    ...
                                    for(Entity e : player.getNearbyEntities(2, 2, 2)) {
                                        ...
                                        if(e instanceof Player) {
                                            ...
                                            if(taskyml.getInt(p.getUniqueId().toString()) < times || times == -1) {
                                                ...
                                                p.sendMessage(Objects.requireNonNull(message.getString("teleport")).replace("%n", "\n"));
    ...
    public Location randomTP(@NotNull World world) {
